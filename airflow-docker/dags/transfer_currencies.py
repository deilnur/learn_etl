...
def transfer_currencies(**kwargs):
    ...
    insert_sql = '''insert into stg.currencies (id, code,name,created,last_update,source_db,processed_dttm)
                        values %s'''
...
with DAG('transfer_currencies', start_date = datetime.datetime(2022,1,1), schedule_interval='@daily', catchup=False, tags = ['transfer']) as dag:
    transfer_currencies = PythonOperator(
        task_id = 'transfer_currencies',
